...
//yts add
			PIR2=0;							//clear USB interrupt flag
			RCONbits.IPEN=1;				//enable priority 
			INTCONbits.GIE=1;
			IPR2bits.USBIP=1;				//connect to high priority 
			PIE2bits.USBIE=1;
//yts add end

            usb_device_state = POWERED_STATE;
//yts        }//end if                           // else wait until SE0 is cleared
...
//yts change 
	...
	else if(UIRbits.ACTVIF && UIEbits.ACTVIE) {USBWakeFromSuspend();return;}

    /*
     * Pointless to continue servicing if the device is in suspend mode.
     */    
	else if(UCONbits.SUSPND==1) return;
     ...
     */
    else if(UIRbits.URSTIF && UIEbits.URSTIE)  {USBProtocolResetHandler();return;}
    
    /*
     * Task C: Service other USB interrupts
     */
    else if(UIRbits.IDLEIF && UIEbits.IDLEIE)    {USBSuspend();;return;}
    else if(UIRbits.SOFIF && UIEbits.SOFIE)      {USB_SOF_Handler();;return;}
	else if(UIRbits.STALLIF && UIEbits.STALLIE)  {USBStallHandler();;return;}
	else if(UIRbits.UERRIF && UIEbits.UERRIE)    {USBErrorHandler();;return;}
     ...
     */
	else if(usb_device_state < DEFAULT_STATE) return;
...
//yts  PIE2bits.USBIE = 0;
    INTCONbits.RBIE = 0;
    /* End Modifiable Section */

}//end USBSuspend

//yts following lines are not changed.
